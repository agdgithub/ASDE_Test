...
bool can_abhimanyu_cross_chakravyuha_recursive(int p, int a, int b, std::vector<int>& k, int current_power, int index) {
    if (index >= 11) {
    ...
    if (current_power >= k[index]) {
        if (can_abhimanyu_cross_chakravyuha_recursive(p, a, b, k, current_power - k[index], index + 1)) {
            return true;
        }
    }
    ...
    if (a > 0) {
        if (can_abhimanyu_cross_chakravyuha_recursive(p, a - 1, b, k, current_power, index + 1)) {
            return true;
        }
    }
    ...
    if (b > 0) {
        if (can_abhimanyu_cross_chakravyuha_recursive(p, a, b - 1, k, p, index)) {
            return true;
        }
    }
...
bool can_abhimanyu_cross_chakravyuha(int p, int a, int b, std::vector<int>& k) {
    return can_abhimanyu_cross_chakravyuha_recursive(p, a, b, k, p, 0);
}
